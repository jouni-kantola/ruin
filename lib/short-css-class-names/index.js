...
module.exports = function(rangeLength) {
    ...
    const charRange = require("./char-range");
    const digits = charRange("0", "9");
    const letters = charRange("A", "Z").concat(charRange("a", "z"));
    const hyphenAndUnderscore = ["-", "_"];

    const validFirstChars = letters;
    const allValidChars = digits.concat(letters).concat(hyphenAndUnderscore);
